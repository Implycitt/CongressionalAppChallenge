    ...
    if (e.key === "Enter") {
        ...
        fetch(`${search_city}?name=${cityName}`)
            ...
            .then(data => {
                ...
                const api_url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,rain,relativehumidity_2m,is_day,precipitation,rain,snowfall,windspeed_10m&hourly=cloudcover,temperature_2m&daily=temperature_2m_max,temperature_2m_min,sunrise,sunset&timezone=${timezone}`;
                fetch(api_url)
                    .then(res => {
                        return res.json();
                    })
